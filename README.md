# VahidKhan Pathan CITS-CSA Learning Log

Welcome to my CITS (Computer Software Application) tracking repository. I use this space to document my day-to-day classwork, practical tasks, and notes from my training program.

## 📚 Resources
* [CITS Ebooks (Google Drive)](https://drive.google.com/drive/folders/1EoN9jtfwT6FOM7l-75YD-x6q9_BIAfmh?usp=drive_link)

## 🗓️ Daily Progress

All my daily exercises and notes are organized into separate folders by date. Below is a log of what was taught in class:

| Day & Date | Topics Covered / Practical Work |
| **Day 11** - 31.08.2026 | *(Assignment 1 on MYSQL)* |
| **Day 10** - 31.08.2026 | *(Nothing Done on this day )* |
| 29th and 30th AUG Week Off |
| **Day 9** - 28.08.2026 | *(Understanding Constrains(Not Null , Unique , Default , Primary Key , Foregin Key)* |
| **Day 8** - 27.08.2026 | *(Understanding Condition based on pattern matches, Inserting Date Values, Constrains(Not Null , Unique , Default , Primary Key , Foregin Key)* |
| 26th holiday milad un nabi |
| **Day 7** - 25.08.2026 | *(Understanding NULL,Executing UPDATE and DELETE,Sorting Query Results)* |
| **Day 6** - 24.08.2026 | *(Relational operators, Logical Oparators, Using Column alias , condition based on range (Between)* |
| 22nd and 24th AUG Week Off |
| **Day 5** - 21.08.2026 | *(Changing the structure of a table,Eliminating duplicate values,Arithematic operators)* |
| **Day 4** - 20.08.2026 | *(Different Types Of SQL Commands)* | *Folder Name* |
| **Day 3** - 19.08.2026 | *(Key featutures of MYSql,Data Types,)* |
| **Day 2** - 18.08.2026 | *(Learning Outcomes)* |
| **Day 1** - 17.08.2026 | *(overall brief about syllabus-CITS_CSA)* |

## 🛠️ Technologies & Subjects Covered
*Module-2 Database Concepts(Started On 18/8/2026-)*
* 
* 
